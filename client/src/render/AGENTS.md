...
| `layers/boss-vfx.ts` | what his attacks LOOK like: the ground TELEGRAPH (drawn under the bodies, in the exact shape `welcome.config.bossMoves` says the server will test), the bar's white-hot TRAIL, the WIND circling a spin, and the crescent a landed blow leaves. `tipAt` re-derives the bar's nose from the row's playhead — it is not on the wire and must not be |
