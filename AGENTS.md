...
- **These pairs are mirrors and change together.** Three are line-for-line; five are rules re-derived on the other side and are just as breakable:
  ...
  - `Room.collect_loot` + `Inventory.add` + `ammo.Reserve` + `armor.Loadout` + `Hotbar` + `medical.Medical` <-> `client/src/game/interaction.ts` (`canStow`, `swapTargetFor`, `replacedBy`, `holdsShield`, `refusal`) — whether a pickup is legal, and what it displaces. A TRADE IS A RULE ABOUT GUN CELLS and not about the belt: steel goes through `swap_blade`, which has no gun cell in it, and a second shield is refused by `Hotbar.add` AND by `swap_weapon` (the trade was the one door past the one-shield rule, and the durability lives in a single field on the body, so the second one arrived holding the first one's damage). Testing `pocket == "hotbar"` instead offered to exchange the player's rifle for the lâmina already in their hand. The client answers locally because the prompt cannot wait for a round trip. FIVE containers now: the pocket refuses when full, a gun cell trades, the BLADE cell always swaps (it has no empty state), a WORN slot refuses only the piece you already have in the same or better condition, and a MEDICAL cell refuses when both are full — it never swaps, because two kits are a quantity rather than alternatives and trading one away would bin the resource the player bent down for
