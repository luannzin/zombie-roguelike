...
- **These pairs are mirrors and change together.** Three are line-for-line; five are rules re-derived on the other side and are just as breakable:
  ...
  - `server/app/boss.py`'s `Move` <-> `client/src/render/layers/boss-vfx.ts` — WHERE THE BAR LANDS. The reach and arc the telegraph is drawn at are shipped in `welcome.config.bossMoves`, straight off the numbers `_in_arc` tests, so the mark on the floor cannot promise a different wedge from the one the swing delivers. What is NOT shipped is the swing's own arc through that wedge: `SWING` in the client mirrors `make_sawyer.py`'s `CHOP_ARC` / `RIP_ARC` so the ribbon leaves the bar instead of floating beside it, and re-timing a clip's arc in the generator means moving that table too
