...
| `projectiles.py` | THINGS THAT TRAVEL, and are therefore things you can WALK AWAY FROM. The flight, the pass-through-and-bill-once rule, and the wall test that runs before the body test. Shared by the boss's crescent and the bloater's spit — a raycast is how guns work and is exactly wrong here |
