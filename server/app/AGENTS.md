...
| `armor.py` | worn plates over FIVE slots, their four materials, what a SET is called and what wearing it MEANS (`Material.tags`), where a blow lands, the figure the HUD draws (`BODY_LAYOUT`), and the shield's numbers. THE FOURTH CONTAINER — see [`docs/design/gear.md`](../../docs/design/gear.md) |
| `ultimates.py` | ONE ULTIMATE PER WEAPON, locked behind a set of armour. The catalog, the tag vocabulary both halves of a build speak, what fills a bar, and three effect blocks `Room` dispatches on. It owns no mechanics and names no combination — a weapon carries tags, a material carries tags, and a row lists the tags it needs. See [`docs/design/ultimates.md`](../../docs/design/ultimates.md) |
| `events.py` | THE NIGHT'S SCRIPT: what happens, when, and the one gate over all of it. Three triggers (a moment, a climbing roll, an action the party took) and a catalog of rows. It owns no mechanics — every effect is a door into machinery `Room` already had — see [`docs/design/events.md`](../../docs/design/events.md) |
| `projectiles.py` | THINGS THAT TRAVEL, and are therefore things you can WALK AWAY FROM. The flight, the pass-through-and-bill-once rule, and the wall test that runs before the body test. Shared by the boss's crescent, the bloater's spit and a player's ultimate volley — a raycast is how guns work and is exactly wrong here. It carries an `owner` and a `look` for its callers and interprets neither |
| `medical.py` | THE FIFTH CONTAINER: the two medical cells, the two kits, and why medicine is not cargo. Both kits are `value=0` — unsellable, unpourable, worth nothing toward a quota — so the greed trade is their WEIGHT, not their price. The heal itself is `Room.use_medical` / `_step_use`, which lands through `Room.heal_player` like every other source; this module is the catalog and the container |
